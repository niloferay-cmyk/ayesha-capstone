...
from src.pipeline.fake_llm import Question, Answer, fake_ask_llm, FakeLLMError
...
async def ask_llm_with_retry(q: Question, tries: int = 3, fail_rate: float = 0.0) -> Answer:
...
log.addHandler(handler)
log.propagate = False  # avoid duplicate logs if root logger also has handlers
